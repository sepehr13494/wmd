  ...
  static BlocWidgetListener defaultBlocListener({
    ...
    return BlocHelper.defaultBlocListener(
      listener: (context, state) {
        ...
        final typeName = AssetTypes.getAssetType(appLocalizations, type);
        if (state is EditAssetSuccess) {
          context
              .read<AssetSummaryCubit>()
              .getSummary(GetSummaryParams(days: 7, assetId: assetId));
          AppRouter().setMainRefreshKey();
          GlobalFunctions.showSnackBar(
              context,
              AppLocalizations.of(context)
                  .assetLiabilityForms_toast_assetEditSuccess
                  .replaceAll("{{assetName}}", typeName),
              type: "success");
          context.pop();
        }
        if (state is DeleteAssetSuccess) {
          GlobalFunctions.showSnackBar(
              context,
              AppLocalizations.of(context)
                  .assetLiabilityForms_toast_assetDeleteSuccess
                  .replaceAll("{{assetName}}", typeName),
              type: "success");
          AppRouter().setMainRefreshKey();
